/**
 * unioncareer.co.il — full group careers page (קבוצת יוניון)
 * Site ID: cmr0rqk7h005b01nz55mhsr2v
 * Load-more: button.js-load-more-jobs (WordPress admin-ajax, 12 per page)
 */
...
const OLD_LEXUS_SITE_ID = "cmqyizf3s003301nzvot8l7ut";
const TOKEN_PATH = path.join(process.cwd(), ".claude", "scrap-token");

// Identical logic to union-lexus; works on any unioncareer department or group page.
...
async function main() {
  // Set company name
  await api("PATCH", `/api/sites/${SITE_ID}`, { companyName: "קבוצת יוניון" });
  console.log("companyName → קבוצת יוניון");

  // Advance from ANALYZING to REVIEW (required before PUT config)
  await api("PATCH", `/api/sites/${SITE_ID}`, { status: "REVIEW" });
  ...
  console.log("config PUT OK (loadMoreSelector: button.js-load-more-jobs)");

  const scrape = (await api("POST", `/api/sites/${SITE_ID}/scrape`, { maxJobs: 50 })) as {
    data?: { id: string };
  };
