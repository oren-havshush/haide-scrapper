(function () {
  try {
    ...
    // Cities: mirrors elbit's own bundle map, but spelled per "CSV files/city.csv"
    // (LRN-LOC-4). *-suffixed ids are absent from elbit's own map — read from each
    // job's description prose, so the site itself renders no location for them.
    var CITY_MAP = {
    ...
    // Dropdown placeholders / work-arrangement flags — not places. Fall through to Area.
    var NOT_A_PLACE = { "0": 1, "7": 1, "131": 1, "DataMigration": 1, "": 1 };
    // Final canonicalisation, applied to whatever any resolution path returns, so the
    // stored value always matches "CSV files/city.csv". לוד and רמלה are not separate
    // entries in that file — it carries the combined "רמלה לוד"; חצור is ambiguous there
    // (חצור הגלילית / חצור אשדוד) and is pinned to חצור הגלילית by standing instruction.
    var CANON = { "לוד": "רמלה לוד", "רמלה": "רמלה לוד", "חצור": "חצור הגלילית" };
    // locationAddress is free text and always a region here; spell it per city.csv.
    var ADDR_ALIAS = {
    ...
    // Single-line fields: collapsing whitespace is correct here.
    function flat(s) {
      var d = document.createElement('div');
      d.innerHTML = unesc(s);
      return (d.textContent || '').replace(/\s+/g, ' ').trim();
    }
    // Body text: the API ships zero real newlines — every line break is an encoded
    // <div>/<br>. Convert those to newlines. NEVER \s+ collapse here (recipe §7).
    ...
    // Requirements heading. Deliberately narrow, and only matched on a SHORT line
    // (recipe §9) so a paragraph merely containing "דרישות" can't trigger a cut.
    // "במסגרת התפקיד" is excluded on purpose — that's responsibilities, not requirements.
    ...
    function splitBody(desc, apiReq) {
      ...
      for (var i = 0; i < lines.length; i++) {
        if (lines[i].trim().length < 60 && REQ_HEAD.test(lines[i])) {
          ...
          // Only accept a cut that leaves both halves substantial.
    ...
    // Many jobs carry no Cities code (only a coarse Area), yet state the site in the
    // body: "לאתר החברה ביקנעם...". Anchored on phrases that actually declare the
    // location — matching any city name anywhere would grab a shuttle-route list
    // ("הסעות: קריות, חיפה, יקנעם, נשר") instead of the real site. Validated against
    // the code map on 325 jobs carrying both: 96.6% agreement (314/325).
    var PROSE_ALIAS = { "יקנעם": "יוקנעם", "נצרת עלית": "נצרת עילית", "תל אביב": "תל אביב-יפו" };
    // ~54% of postings are English-only; without this they fall back to a region.
    var EN_CITY = {
    ...
    // Most postings end with an explicit location hashtag ("#Netanya", "#Ramat HaSharon").
    // Only a tag that resolves to a known city is accepted, which drops noise like
    // "#LI-DNI". Agrees with the Cities code on 450/467 jobs carrying both (96.4%).
    ...
    // Precedence is most-precise-first, matching what elbit's own page renders
    // ("מיקום: כרמיאל" comes from the Cities code). locationAddress must NOT come
    // first: it is often just a region ("צפון") on a job that also carries a precise
    // Cities code, and checking it first threw the city away (jobCode 1095).
    function pickLocation(job, descText) {
      ...
      // 2. explicit "#City" tag at the end of the posting
      var hc = hashCity(descText);
      if (hc) return hc;
      // 3. city named in the body prose
      var pc = proseCity(descText);
      if (pc) return pc;
      // 4. free-text locationAddress (a region in practice)
